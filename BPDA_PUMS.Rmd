...
##clean population
```{r}
age <- brazil_pop_2018[,c("SPORDER","ST","AGEP")]
age$group <- "Population"
t14 <- age %>% group_by(ST) %>% count(group) %>% 
...
t1 <- age %>% group_by(ST) %>% count(group) %>% 
  pivot_wider(names_from = ST, values_from = n)
...
t2 <- sex %>% group_by(ST) %>% count(group) %>% pivot_wider(names_from = ST, values_from = n)
...
t3 <- mar %>% group_by(ST) %>% count(group) %>% 
  pivot_wider(names_from = ST, values_from = n)
...
t4<-cit %>% group_by(ST) %>% count(group) %>% 
  pivot_wider(names_from = ST, values_from = n)
...
t5 <- yoe %>% group_by(ST) %>% count(group) %>% 
  pivot_wider(names_from = ST, values_from = n)
...
t7 <- edu %>% group_by(ST) %>% count(group) %>% 
  pivot_wider(names_from = ST, values_from = n)
...
t8 <- cow %>% group_by(ST) %>% count(group) %>% 
  pivot_wider(names_from = ST, values_from = n)
...
```{r}
indp <- brazil_pop_2018[,c("ST","INDP")]
...
options(dplyr.summarise.inform = FALSE)
t9<- indp %>% group_by(ST,INDP) %>% summarise(N=n())
t9 <-filter(t9, !(INDP %in% c("0169", "9920","3370","7580","3380","7590",
                                 "3390", "8180" )))
t9 <- pivot_wider(t9, names_from = ST, values_from = N) 
t9<-column_to_rownames(t9, var = "INDP")
```
##Poverty Status
```{r}
povpip <- brazil_pop_2018[,c("ST","POVPIP")]
...
t10<-povpip %>% group_by(ST) %>% count(group) %>% 
  pivot_wider(names_from = ST, values_from = n)
t10<-column_to_rownames(t10, var = "group")
```
##Median Personal Earnings
```{r}
wagp <- brazil_pop_2018[,c("ST","WAGP")]
t12 <- filter(wagp,WAGP>0)
t11<-wagp%>%group_by(ST) %>%summarise(across(where(is.numeric), median))%>%
  pivot_wider(names_from = ST, values_from = WAGP)

t12 %<>% group_by(ST) %>%summarise(across(where(is.numeric), median)) %>%
  pivot_wider(names_from = ST, values_from = WAGP)
...
household_2018 <- get_pums(
  variables = c("POBP","HHT","NPF","POVPIP","OCPIP","GRPIP","ACR","HINCP",
                "FINCP"),
  state = "all",
  survey = "acs1",
  year = 2018
...
##Clean householder 
```{r}
hht<- brazil_hou_2018[,c("ST","HHT")]
hht<-filter(hht,!(HHT=="b"))
hht$group <- "Householder"
b1 <- hht %>% group_by(ST) %>% count(group) %>% pivot_wider(names_from = ST, values_from = n) 
...
b2<- hht1 %>% group_by(ST) %>% count(group) %>% pivot_wider(names_from = ST, values_from = n)
...
ocpip <- brazil_hou_2018[,c("ST","OCPIP")]
ocpip$group <-"Owner Occupied Unit"
b3<- ocpip %>% group_by(ST) %>% count(group) %>%
  pivot_wider(names_from = ST, values_from = n)
b3<-column_to_rownames(b3, var = "group")

grpip<- brazil_hou_2018[,c("ST","GRPIP")]
grpip$group<-" Renter Occupied Unit"
b4<- grpip %>% group_by(ST) %>% count(group) %>% 
  pivot_wider(names_from = ST, values_from = n)
...
b6 <- ocpip %>% group_by(ST) %>% count(group) %>% 
  pivot_wider(names_from = ST, values_from = n) 
...
b7 <- grpip %>% group_by(ST) %>% count(group) %>% 
  pivot_wider(names_from = ST, values_from = n) 
...
```{r}
hincp <- brazil_hou_2018[,c("ST","HINCP")]

b9<- hincp %>% group_by(ST) %>%summarise(across(where(is.numeric), median)) %>%
  pivot_wider(names_from = ST, values_from = HINCP)
rownames(b9) <- " Median Household Income"
```

##Median Family Income
```{r}
fincp <- brazil_hou_2018[,c("ST","FINCP")]

b10<- fincp %>% group_by(ST) %>%summarise(across(where(is.numeric), median)) %>%
  pivot_wider(names_from = ST, values_from = FINCP)
